...
void SQLHelper::init()
{
    ...
    m_dbPath = pFileUtils->getWritablePath();
...
void SQLHelper::updateKategoria(KategoriaModel *pKat)
{
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE)
          log("ERROR UPDATING KATEGORIA");
    
    sqlite3_finalize(stmt);

}

Vector<KategoriaModel *> * SQLHelper::queryKategoriak(int idKategoria)
{
    sqlite3_stmt *stmt;
